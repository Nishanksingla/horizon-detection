...
Note: Before running the script please install the dependencies using the `requirements.txt` file. Simple run `pip install -r requirements.txt` in your terminal. I would suggest to create a virtual env and install these dependencies in the virtual env. I have used Python 3.9.7 version to develop and to execute the script.
...
# Evaluation Metric: 
I have used Euclidean distance as a evaluation metric. It calculates the distance between 2 points. I think it's a good metric to use to evaluate the prediction of the points.
